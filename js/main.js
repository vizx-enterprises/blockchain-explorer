...
function getAndDisplayLastBlockHeader() {
  $.ajax({
    ...
    success: function (data) {
      ...
      $('#blockchainReward').text(numeral(data.reward / Math.pow(10, ExplorerConfig.decimalPoints)).format('0,0.0000') + ' ' + ExplorerConfig.ticker)
      $('#blockchainTransactions').text(numeral(data.alreadyGeneratedTransactions).format('0,0'))
      $('#blockchainCirculatingSupply').text(numeral(data.alreadyGeneratedCoins / Math.pow(10, ExplorerConfig.decimalPoints)).format('0,0.0000') + ' ' + ExplorerConfig.ticker)
      $('#blockchainTotalSupply').text(numeral(ExplorerConfig.maxSupply / Math.pow(10, ExplorerConfig.decimalPoints)).format('0,0.0000') + ' ' + ExplorerConfig.ticker)
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        var txn = data[i]
        table.row.add([
          numeral(txn.amount / Math.pow(10, ExplorerConfig.decimalPoints)).format('0,0.0000'),
          numeral(txn.fee / Math.pow(10, ExplorerConfig.decimalPoints)).format('0,0.0000'),
