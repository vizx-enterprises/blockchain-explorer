$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function (block) {
      ...
      $('#blockBaseReward').text(numeral(block.baseReward / Math.pow(10, ExplorerConfig.decimalPoints)).format('0,0.0000000') + ' ' + ExplorerConfig.ticker)
      $('#blockReward').text(numeral(block.reward / Math.pow(10, ExplorerConfig.decimalPoints)).format('0,0.0000000') + ' ' + ExplorerConfig.ticker)
      $('#blockTransactionSize').text(numeral(block.transactionsCumulativeSize).format('0,0') + ' bytes')
      $('#blockTransactionFees').text(numeral(block.totalFeeAmount / Math.pow(10, ExplorerConfig.decimalPoints)).format('0,0.0000000') + ' ' + ExplorerConfig.ticker)
      ...
      for (var i = 0; i < block.transactions.length; i++) {
        ...
        transactions.row.add([
          ...
          numeral(txn.amount_out / Math.pow(10, ExplorerConfig.decimalPoints)).format('0,0.0000000'),
